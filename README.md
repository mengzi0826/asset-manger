...
- 资产摘要：累计变化、浮动盈亏、加权年化收益率、即将到期列表（60 天内）
...
- 持仓明细，按市场分组（沪深 A 股 → 港股 → 美股），每行附 mini sparkline
...
lib/
  ...
  history.ts                 # 变动日志 + 每日快照 + 证券盈亏重建
  fx.ts                      # 汇率拉取 / 缓存 / 换算
  stocks.ts                  # 股票代码解析 + 价格刷新
  advisor.ts                 # 智能建议规则引擎
