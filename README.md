...
- 资产摘要：四个指标瓦片（累计变化、浮动盈亏、**证券今日**盈亏、加权年化）+ 即将到期列表（60 天内）；「证券今日」由落库涨跌字段汇总；**周六、日**显示 **—**（休市不调行情）；无证券持仓（或份额均为 0）时亦为 **—**
...
- KPI 条：证券总市值、**今日盈亏**（参与汇总的标的数）、浮动盈亏（成本回报率）、按市场分组（沪深 A 股 / 港股 / 美股）
- 证券总市值走势图（来自每日组合快照）
...
- **股票价格**：工作日每日 10:00 / 14:00（北京时间）锚点内访问页面时后台触发自动刷新；**周六、日不调接口**，证券页与总览「今日」为 **—**。刷新成功时写入 `current_price` 与接口返回的涨跌字段；单条失败或额度错误**不会 UPDATE 该行**，不会清空其它标的已有数据。`change_quote_date` 仅作会话日记录。未配置 AppKey 时跳过。
...
| `asset`              | 资产明细，含 `symbol`；证券类另有 `change_amount` / `change_percent`、`change_quote_date`（接口会话日，仅记录）、`change_updated_at`（历史兼容） |
| `fx_rate`            | 汇率缓存                              |
...
**股票价格未更新**  
→ 确认「设置」中已配置聚合数据股票 AppKey；**工作日**在北京时间 10:00 / 14:00 锚点内访问页面会后台自动刷新，也可「设置 → 股票价格」手动刷新。**周六、日不调接口。**

**总览「证券今日」为「—」**  
→ 无份额大于 0 的证券持仓，或当前为**北京周六、日**（休市展示）。

**部分标的「今日」为「—」、其它正常**  
→ 该次刷新里该标的接口失败（含额度限制），**失败不会清空库里已有涨跌**；下一波刷新成功即可恢复。
