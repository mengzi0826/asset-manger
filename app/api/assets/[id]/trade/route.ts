...
const tradeSchema = z.object({
  ...
  quantity: z
    ...
    .refine((v) => Number.isFinite(v) && v > 0, "成交股数必须大于 0"),
  price: z
    .union([z.number(), z.string()])
    .transform((v) => Number(v))
  ...
  unit_cost: z
    .union([z.number(), z.string()])
    .transform((v) => Number(v))
    .refine((v) => Number.isFinite(v) && v >= 0, "均价无效"),
  cash_asset_id: z
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (security.category_code !== "securities") {
      return NextResponse.json({ error: "该资产不是证券，无法交易" }, { status: 400 });
    }

    const body = await req.json();
    const parsed = tradeSchema.parse(body);

    const qtyDelta = parsed.quantity;
    const tradeValue = qtyDelta * parsed.price;
    const curQty = security.quantity ?? 0;

    if (parsed.side === "sell" && qtyDelta > curQty + 1e-9) {
      return NextResponse.json(
        { error: `减持股数超过当前持仓（当前 ${curQty}）` },
        { status: 400 }
      );
    }
    ...
    const run = db.transaction(() => {
      ...
      ).run({ id, quantity: nextQty, unit_cost: parsed.unit_cost, updated_at: now });
      ...
      if (cash) {
        ...
        const nextAmount =
          (cashBefore.amount ?? 0) + (parsed.side === "buy" ? -tradeValue : tradeValue);
        db.prepare(
